  ...
  renderNewCardForm() {
    return (
      <Jumbotron className="position_relative">
        <form onSubmit={() => localStorage.setItem('addCard', 1)} action='https://sbcheckout.PayFort.com/FortAPI/paymentPage' method="post">
          <Row>
            <div className="user_data center_div">
              <span className="one_dollar_msg">{gettext('ONE-DOLLAR-MSG')}</span>
          ...
          <div className="actions">
            <Button type="submit" onClick={() => this.convert()} bsStyle="primary">{gettext('ADD')}</Button>
